...
with tab3:
    ...
    if st.button("Generate Test Blocks"):
        ...
        try:
            nb_bricks, nb_assemblies = generate_test_blocks()
            st.success(f"Successfully generated {nb_bricks} shape test bricks and {nb_assemblies} assembly sequence in `output/debug/`")
        except Exception as e:
            import traceback
